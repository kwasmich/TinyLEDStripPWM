...
static void pwm() {
    ...
    if (cnt < 4) {
        if (r > cnt) {
            BIT_SET(PORTB, _BV(R_PIN));
        }

        if (g > cnt) {
            BIT_SET(PORTB, _BV(G_PIN));
        }

        if (b > cnt) {
            BIT_SET(PORTB, _BV(B_PIN));
        }
    }
...
static uint8_t i2cReadFromRegister(uint8_t reg) {
    switch (reg) {
        case '@':
            return eeprom_read(0x00);     // this is pretty much useless but for completeness
...
static void i2cWriteToRegister(uint8_t reg, uint8_t value) {
    switch (reg) {
        ...
        case 'G':
            eeprom_write(0x02, value);
            break;

        case 'B':
            eeprom_write(0x03, value);
            break;
